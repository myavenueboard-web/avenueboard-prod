...
const handleUnlock = (e: React.FormEvent<HTMLFormElement>) => {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#F7F6F3] pt-[96px] font-sans text-[#0F172A] sm:pt-[110px]">
      ...
      {!isUnlocked && (
  <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#F7F6F3]/95 px-4 backdrop-blur-xl">
    <form
      onSubmit={handleUnlock}
      className="w-full max-w-[420px] rounded-[32px] border border-zinc-200 bg-white p-7 text-center shadow-[0_30px_120px_rgba(15,23,42,0.12)]"
    >
      <p className="text-[12px] font-semibold uppercase tracking-[0.24em] text-[#B9476D]">
        AvenueBoard
      </p>

      <h2 className="mt-4 text-[30px] font-semibold tracking-[-0.06em] text-[#0F172A]">
        Product locked for admin access.
      </h2>

      <p className="mx-auto mt-3 max-w-[320px] text-[14px] leading-6 text-zinc-500">
        Enter passcode to preview the landing page.
      </p>
      ...
      <input
        ...
        className="mt-7 h-12 w-full rounded-2xl border border-zinc-200 bg-[#FAFAFA] px-4 text-center text-[15px] font-semibold tracking-[0.2em] text-[#0F172A] outline-none transition focus:border-[#B9476D] focus:bg-white"
      />

      {passcodeError && (
        <p className="mt-3 text-[13px] font-medium text-[#B9476D]">
          {passcodeError}
        </p>
      )}
      ...
      <button
        type="submit"
        className="mt-5 h-12 w-full rounded-2xl bg-[#0F172A] text-[14px] font-semibold text-white transition hover:bg-[#1E293B]"
      >
        Unlock Preview
      </button>
    </form>
